...
function checkCondProc(proc){
	// parse用Obj生成関数
	var createCondprocObject = (vls, dcs) => {
...
// 仮ダメージ順に並び替える
function calcPreDamageAndSort(cs, c_obj){
	// c_objから値を取り出す
	var e_attr_set = c_obj ? c_obj.e_attr : -1;
	var is_awake_adv = c_obj ? c_obj.aw_adv : true;
	var is_awake_atk = c_obj ? c_obj.aw_atk : true;
